...
import './libraries/v6/WhaleswapLibrary.sol';
...
contract WhaleswapRouter is IWhaleswapRouter {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = WhaleswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = WhaleswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'WhaleRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = WhaleswapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidity(
        ...
        liquidity = IWhaleswapPair(pair).mint(to);
    }
    ...
    function removeLiquidity(
        ...
        address pair = WhaleswapLibrary.pairFor(factory, tokenA, tokenB);
        IWhaleswapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IWhaleswapPair(pair).burn(to);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IWhaleswapPair(WhaleswapLibrary.pairFor(factory, input, output)).swap(
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(WhaleswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = WhaleswapLibrary.sortTokens(input, output);
            IWhaleswapPair pair = IWhaleswapPair(WhaleswapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(WhaleswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
